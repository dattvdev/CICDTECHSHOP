...
namespace TechShop.Controllers
{
    public class HomeController : Controller
    {
		...
		[HttpPost]
		public async Task<IActionResult> CartWithLocal([FromBody] List<productColorLocal> productColorLocals)
		{
			if (productColorLocals == null || !productColorLocals.Any())
			{
				return BadRequest("No cart items received.");
			}
			var products = new List<Order>();
			foreach (var item in productColorLocals)
			{
				var getProduct = await _context.ProductColors.Include(o => o.ProductHardware).ThenInclude(c => c.Product).Where(c => c.Id.ToString().Equals(item.ColorId)).FirstOrDefaultAsync();
				if(getProduct is not null)
				{
					var order = new Order();
                    order.Quanitity = (int)item.Quantity;
                    order.ProductColor = getProduct;
					products.Add(order);
				}
			}
        ...
        [HttpPost]
        public async Task<IActionResult> AddLocalCart([FromBody] List<productColorLocal> productColorLocals)
        {
            if (productColorLocals == null || !productColorLocals.Any())
            {
                return BadRequest("No cart items received.");
			...
			try
			{
                ...
                foreach (var item in productColorLocals)
                {
                    ...
                    if (getProduct is not null)
                    {
						var orderExist = await _context.Orders.FirstOrDefaultAsync(c => c.ProductColorId.ToString().Equals(getProduct.Id.ToString()) && c.CustomerId.ToString().Equals(cus.ID.ToString()));
						if(orderExist == null)
						{
                            var order = new Order();
                            order.Quanitity = (int)item.Quantity;
                            order.ProductColorId = getProduct.Id;
						...
						{
							orderExist.Quanitity = item.Quantity.Value;
							_context.Orders.Update(orderExist);
						}
      
                    }
                }
                await _context.Orders.AddRangeAsync(products);
                await _context.SaveChangesAsync();
            }
			catch
			{
